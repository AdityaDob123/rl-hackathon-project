    ...
    def step(self, action: Action) -> Tuple[Observation, Reward, bool, Dict]:
        try:
            ...
            if self.state_manager.done:
                ...
                done_reward = Reward(value=0.0, components={}, message="Episode already done.")
                return done_obs, done_reward, True, {
                    "task_id": done_obs.task_id,
                    "difficulty": done_obs.difficulty,
                    "final_score": sanitize_score(0.0),
            ...
            final_score = grade_action(action, self.current_task, self.current_step_index) if valid else sanitize_score(0.0)
            ...
            fallback_obs = self._fallback_observation()
            fallback_reward = Reward(value=0.0, components={}, message="Error fallback.")
            self.state_manager.load_task(fallback_obs)
            return fallback_obs, fallback_reward, True, {
